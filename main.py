...
def func(vtu_num):
    ...
    d =webdriver.Chrome(executable_path="C:\\browser_drivers\\chromedriver.exe")
    ...
    names = d.find_element_by_xpath('//*[@id="ContentPlaceHolder1_lblStudentName"]')
    sems_comp=d.find_element_by_xpath('//*[@id="ContentPlaceHolder1_lblSemester"]')
    ti=int(sems_comp)*15
    update.message.reply_text("Estimated Waiting Time : "+str(ti)+" secs")
    sem_wise = names+"/n"
    sgpa,ccode,cname,cgrade,ccredit,cgpoints=[],[],[],[],[],[]
    sem_det=""
    for semm in range(1,sems_comp):
        scode = []
        sname = []
        sgrade = []
    ...
    sem_wise=sem_wise+"Total Cgpa : "+str(cgpa2*10)[0:4]
    return sem_wise
